...
def ensure_go2rtc(index, force=False):
    name = f"camera{index + 1}"
    with go2rtc_lock:
        if go2rtc_ready.get(index) and not force:
            return name
        url = fresh_stream(index, force=True)
        diag(f"[camera {index + 1}] Handing fresh RTMP session to go2rtc")
        r = requests.put("http://127.0.0.1:1984/api/streams",
                         params={"name": name, "src": url}, timeout=8)
        if not r.ok:
            raise RuntimeError(f"go2rtc rejected stream: HTTP {r.status_code} {r.text[:200]}")
        ...
        try:
            ...
            proc = subprocess.Popen(
                ["ffmpeg", "-hide_banner", "-loglevel", "error",
                 ...
                 "-i", url, "-an", "-vf", "fps=10,scale=1280:720:force_original_aspect_ratio=decrease", "-q:v", "4",
                 "-f", "mjpeg", "pipe:1"],
...
def home():
    ...
    .video img,.video video{width:100%;height:100%;object-fit:contain}.ptz{display:grid;grid-template-columns:68px 68px 68px;gap:8px;justify-content:center;margin:14px 0}
...
def go2rtc_mp4(index):
    ...
    try:
        ...
        upstream = requests.get("http://127.0.0.1:1984/api/stream.mp4",
                                params={"src": name}, stream=True, timeout=(8, None))
        if not upstream.ok:
            with go2rtc_lock: go2rtc_ready.pop(index, None)
            raise RuntimeError(f"go2rtc playback failed: HTTP {upstream.status_code}")
        def chunks():
            try:
                ...
                upstream.close()
        return Response(chunks(), mimetype=upstream.headers.get("Content-Type","video/mp4"),
                        headers={"Cache-Control":"no-store"})
    except Exception as e:
        diag(f"[camera {index + 1}] go2rtc error: {type(e).__name__}: {e}")
        with go2rtc_lock: go2rtc_ready.pop(index, None)
        return jsonify(ok=False,error=str(e)),500
